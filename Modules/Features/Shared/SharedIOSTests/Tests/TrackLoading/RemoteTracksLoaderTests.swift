...
final class RemoteTracksLoaderTests: XCTestCase {
  ...
  func test_load_deliversArtistsOn200HTTPResponseWithJSONItems() async throws {
    ...
    let artist1 = makeAlbum(id: 0, title: "Some title", imageURL: .anyURL())
    let artist2 = makeAlbum(id: 1, title: "Another title", imageURL: .anyURL())
    let artists = [artist1.entity, artist2.entity]
    let json = makeAlbumJSON([artist1.json, artist2.json])
...
extension RemoteTracksLoaderTests {
  private typealias Entity = Track
  private typealias SUT = RemoteTracksLoader
...
extension RemoteTracksLoaderTests {
  ...
  private func makeAlbum(
    ...
    let album = Entity(id: id, title: title)
    var json = [String: Any]()
    json["id"] = album.id
  ...
  private func anyValidJSONData() -> Data {
    makeAlbumJSON(
      [
        makeAlbum(id: 0, title: "Thrill Of The Chase", imageURL: .anyURL()).json,
      ]
    )
  }
  ...
  private func makeAlbumJSON(_ artists: [[String: Any]]) -> Data {
